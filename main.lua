...
-- :ddddddddddhyyddddddddddd: Modified: 2015-03-30 23:38:17
...
function love.load()
	...
	for i=1,max do
		...
		-- 		table.insert(t1, {n = 0, x = x * size, y = y * size})
		-- 	else
				table.insert(t1, {n = math.random(0, 255), x = x * size, y = y * size})
		-- 	end
		-- end
		y = y + 1
...
function love.update(dt)
	...
	for i=1,max do
		...
		-- print(exec((t1[i].x * t1[i].y + time) % 255, 1))
			...
			love.graphics.rectangle("fill", t1[i].x + 1, t1[i].y + 1, size - 2, size - 2)
			-- love.graphics.circle("line", t1[i].x + 1, t1[i].y + 1, (size / 2) - 2)
			-- t1[i].n = time
			love.setBlank()
	end
